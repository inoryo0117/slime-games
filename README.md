...
`assets/kenney_tower-defense-top-down/` は [Kenney](https://kenney.nl/assets/tower-defense-top-down) の Tower Defense (top-down) Pack（CC0）です。同梱の `License.txt` を参照してください。`chibi-sprites.png`（ちびキャラの絵）と `chibi-enemies.png`（ちびキャラ版の敵とボスの絵）は、画像生成AIで描かせたものをドット単位で整えたものです。それ以外のグラフィックはすべてコードで描いています。
